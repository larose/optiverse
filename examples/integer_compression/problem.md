...
Your codebase is a directory. The `.go` files at its root belong to `package candidate`, and together they must define:
...
You may split the package across as many files as you like; add, rename or delete them freely. Keep them at the root: files in subdirectories are not compiled.
...
Each run is a **fresh process**. Nothing survives from one to the next: no package-level state, no cached tables, no files.
